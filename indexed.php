...
// Handle reposts (only for logged-in users)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['repost']) && $is_logged_in) {
...
// Handle reactions via AJAX (only for logged-in users)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_react']) && $is_logged_in) {
    $post_id = $_POST['post_id'];
    $type = $_POST['type'];
    $user_id = $_SESSION['user_id'];
...
// Handle comments (only for logged-in users)
...
// Handle replies (only for logged-in users)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reply']) && $is_logged_in) {
    $post_id = $_POST['post_id'];
                ...
                <div class="card mb-3" id="post-<?php echo $post['id']; ?>">
                    <div class="card-body">
                        ...
                        <div class="mt-3 d-flex align-items-center">
                            ...
                            // Check if current user has reacted (only if logged in)
                            $user_reaction = null;
                            if ($is_logged_in) {
                                $stmt = $pdo->prepare("SELECT type FROM reactions WHERE post_id = ? AND user_id = ?");
                            ...
                            <?php else: ?>
                                <button class="btn btn-sm btn-outline-primary me-2" disabled title="Please login to react">
                                    ❤️ <span class="like-count"><?php echo $like_count; ?></span>
                                </button>
                            <?php endif; ?>
                            ...
                            <?php else: ?>
                                <button class="btn btn-sm btn-outline-danger me-2" disabled title="Please login to react">
                                    💔 <span class="dislike-count"><?php echo $dislike_count; ?></span>
                                </button>
                            <?php endif; ?>
                            ...
                            <?php else: ?>
                                <button class="btn btn-sm btn-outline-secondary me-2" disabled title="Please login to repost">
                                    🔄 <span class="repost-count"><?php echo $post['repost_count']; ?></span>
                                </button>
                            <?php endif; ?>
                        ...
                        <div class="comments-section mt-3" id="comments-<?php echo $post['id']; ?>" style="display: none;">
                            ...
                            <?php else: ?>
                                <div class="alert alert-info mt-3">
                                    Please <a href="login.php">login</a> to comment on this post.
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
...
<script>
    $(document).ready(function () {
        // Handle reactions with AJAX (only if user is logged in)
        <?php if ($is_logged_in): ?>
        ...
        // Handle repost with AJAX (only if user is logged in)
        <?php if ($is_logged_in): ?>
